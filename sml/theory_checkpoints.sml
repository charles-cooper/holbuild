...
val runtime_step_plan_lines =
  ["fun holbuild_frag_type (TacticParse.FAtom (TacticParse.LSelectGoal _)) = \"select\"",
   ...
   "fun holbuild_frag_text body (TacticParse.FAtom a) = (case a of TacticParse.LReverse => \"Tactical.REVERSE_LT\" | _ => let val raw = (case (TacticParse.topSpan a, holbuild_alt_span a) of (SOME sp, _) => holbuild_substring body sp | (NONE, SOME sp) => holbuild_substring body sp | _ => \"\") in case a of TacticParse.Subgoal _ => if String.size raw > 0 andalso String.sub(raw, 0) = #\"`\" then \"sg \" ^ raw else raw | _ => raw end)",
